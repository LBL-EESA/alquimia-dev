...
int main(int argc, char* argv[]) 
{
  ...
  BatchChemDriverInput* input = BatchChemDriverInput_New(input_file);
  if (input == NULL)
    alquimia_error("batch_chem: error encountered reading input file '%s'.", input_file);
  ...
  // Create a BatchChemDriver from the parsed input.
  BatchChemDriver* batch_chem = BatchChemDriver_New(input);

  // Run the simulation.
  int status = BatchChemDriver_Run(batch_chem);
  ...
  if (output != NULL)
  {
    ...
    BatchChemDriver_GetSoluteAndAuxData(batch_chem, &final_time, &var_names, &var_data);
